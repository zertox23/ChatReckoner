...
class RankPoll(commands.Cog):
    ...
    async def pollsubmit (self,interaction: discord.Interaction,user:discord.Member,conclusion:app_commands.Choice[int],reason:str):
        if interaction.channel.id != tier_submit_channel:
            ...
            await interaction.response.send_message(embed=create_embed("","Poll Submitted.waiting for admin approval",color=discord.Color.green()), ephemeral=True)
    ...
    async def accept_poll(self,interaction:discord.Interaction,message_link:str):
            ...
            if interaction.channel.id != admin_tier_submit:
                ...
                await interaction.response.send_message(
                    embed=create_embed("Error", f"Wrong Channel,Check {channel.mention}", color=discord.Color.red()),
                    ephemeral=True)
            else:
                if original_message.embeds:
                    ...
                    ic(poll_for_value)
                    poll_db = DbStruct.tierpolls(message_id=message.id,submitted_by=submitted_by,voted_user=poll_for_user_id)
    ...
    async def poll_stats(self, interaction: discord.Interaction, message_link: str):
        ...
        try:
            ...
            poll = session.query(DbStruct.tierpolls).filter(DbStruct.tierpolls.id == int(message.id)).first()
            if poll:
                poll_ratio_message = create_ratio_string(upvotes=poll.upvotes,downvotes=poll.downvotes)
                embed = discord.Embed(title="Poll Overview",colour=discord.Color.random())
                embed.add_field(name="Poll ID",value=str(poll.id))
                embed.add_field(name="Stats requested by",value=interaction.user.mention)
                embed.add_field(name="Poll made by",value=self.bot.get_user(poll.submitted_by).mention)
                embed.add_field(name="**Results**",value="",inline=False)
                embed.add_field(name="Win ratio",value=poll_ratio_message,inline=False)
