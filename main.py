...
class utils:
    ...
    def get_all_members() -> (
        list
    ):  # gets the necessary data to make a DbStruct.member object
        members_info = []
...
async def update_members(interaction: discord.Interaction):
    try:
        ...
        await interaction.followup.send(
            embed=create_embed(
                " ", "Success server status updated", color=discord.Color.green()
            )
        )
    except Exception as e:
        logger.error(str(e))
    ...
    try:
        ...
        if res:
            ...
            await interaction.followup.send(
                embed=create_embed(
                    " ",
                    f"Success. Added {n} Votes to {user.mention}",
            ...
            await interaction.response.defer()
            await interaction.followup.send(
                embed=create_embed("Error", "Failed.", color=discord.Color.red())
            )
    except Exception as e:
        await interaction.response.defer()
        await interaction.followup.send(
            embed=create_embed("Error", f"Failed,{str(e)}", color=discord.Color.red())
        )


@bot.tree.command(name="down_vote")
@app_commands.describe(user="user to add votes to", n="how many votes to add")
@commands.has_permissions(administrator=True)
    ...
    try:
        ...
        if res:
            ...
            await interaction.followup.send(
                embed=create_embed(
                    " ",
                    f"Success. Removed {n} Votes to {user.mention}",
                    color=discord.Color.green(),
                )
            )
        else:
        ...
        if role.name in tiers:
            ...
            if next_role_index < 0:
                ...
                await dis_member.add_roles(
                    discord.utils.get(
                        bot.get_guild(Guild).roles, name=str(tiers[next_role_index])
                    )
                )  # Give lower Rank
                await dis_member.remove_roles(
                    discord.utils.get(
                        bot.get_guild(Guild).roles, name=str(tiers[current_role_index])
                    )
                )  # Remove last Rank
                await interaction.response.defer()
                await interaction.followup.send(
                    embed=create_embed(
                        " ",
                        f"Deranked {user.mention} successfully",
                        color=discord.Color.green(),
                    )
                )
        ...
        if role.name in tiers:
            ...
            if len(tiers) <= next_role_index:
                ...
                await interaction.followup.send(
                    embed=create_embed(
                        " ",
                        f"upranked {user.mention} successfully",
                        color=discord.Color.green(),
                    )
                )
...
async def show_user_info(interaction: discord.Interaction, user: discord.Member):
    member = (
        session.query(DbStruct.member)
        .filter(DbStruct.member.user_id == user.id)
        .first()
    )
    if member:
        ...
        embed.add_field(
            name="messages Sent", value=str(member.messages_sent), inline=False
        )
        await interaction.response.defer()
        # asyncio.sleep()
